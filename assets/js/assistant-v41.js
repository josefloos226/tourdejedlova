...
const API_URL="https://tdj-isabel-api.josef-loos226.workers.dev/";
// V4.1: krátkodobá paměť dialogu — posledních 10 zpráv (cca 5 výměn).
const chatHistory=[];
...
async function ask(q){
 ...
 typing.className="tdj-a-msg bot";
 typing.textContent="Přemýšlím…";
 chat.appendChild(typing);chat.scrollTop=chat.scrollHeight;
 try{
   const r=await fetch(API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:q,history:chatHistory})});
   if(!r.ok)throw new Error(`API ${r.status}`);
   const data=await r.json();
   if(!data||!data.answer)throw new Error("Chybí odpověď API");
   typing.remove();say(data.answer);
   remember("user",q);
   remember("model",data.answer);
 }catch(err){
   console.warn("ISABEL API fallback:",err);
   const fallback=answer(q);
   typing.remove();say(fallback);
   remember("user",q);
